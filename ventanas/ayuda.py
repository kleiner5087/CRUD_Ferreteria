...
def crear_ventana_ayuda(ventana_padre):
    global ventana_ayuda
    if ventana_ayuda is None or not ventana_ayuda.winfo_exists():
        ventana_ayuda = tk.Toplevel(ventana_padre)
        ventana_ayuda.title("Ayuda")
        ventana_ayuda.geometry("900x910")
        ventana_ayuda.resizable(True, True)
        ventana_ayuda.regresar = ventana_padre
        ...
        tk.Label(info_frame, text="Desarrollado por:", font=("Roboto", 12), fg=color_subtitulo, bg=color_fondo).pack(pady=5)
        tk.Label(info_frame, text="Jesús Adrian López Magaña | 222H17059", font=("Roboto", 12), fg=color_subtitulo, bg=color_fondo).pack(pady=5)
        tk.Label(info_frame, text="Ángel Ariel Madrigal Ricardez | 222H17076", font=("Roboto", 12), fg=color_subtitulo, bg=color_fondo).pack(pady=5)
        tk.Label(info_frame, text="Leonardo Santamaria Sánchez | 222H17123", font=("Roboto", 12), fg=color_subtitulo, bg=color_fondo).pack(pady=5)
        tk.Label(info_frame, text="Materia: Laboratorio De Diseño De Software", font=("Roboto", 12), fg=color_subtitulo, bg=color_fondo).pack(pady=5)
        tk.Label(info_frame, text="Docente: Rafael Mena de la Rosa", font=("Roboto", 12), fg=color_subtitulo, bg=color_fondo).pack(pady=5)

        desc_frame = tk.Frame(frame_contenido, bg=color_fondo)
        desc_frame.pack(fill='both', expand=True)

        tk.Label(desc_frame, text="Descripción del Sistema", font=("Roboto", 14, "bold"), fg=color_subtitulo, bg=color_fondo).pack(pady=(20, 10))
...
"""
        ...
        text_widget = tk.Text(desc_frame, wrap=tk.WORD, font=("Roboto", 12), bg=color_fondo, fg=color_texto, height=20, borderwidth=0)
        ...
        boton_regresar_ayuda.place(x=100, y=5)

        ventana_ayuda.withdraw()
    return ventana_ayuda
